 ...
 */
public class MakeReactantOrProductInExistingReactionEdit implements
		IUndoRedoable {
	 ...
	 */
	public MakeReactantOrProductInExistingReactionEdit(
	...
	public void redo() {
		...
		IReaction reaction = ReactionSetManipulator.getReactionByReactionID(
				chemModel.getReactionSet(), reactionID);
		if (reaction == null) {
	...
	public void undo() {
		...
		if (reactantOrProduct) {
			reactantsorproducts = ReactionSetManipulator
					.getReactionByReactionID(chemModel.getReactionSet(),
							reactionID).getReactants();
		} else {
			reactantsorproducts = ReactionSetManipulator
					.getReactionByReactionID(chemModel.getReactionSet(),
							reactionID).getProducts();
		}
		final IReaction reaction = ReactionSetManipulator
				.getReactionByReactionID(chemModel.getReactionSet(), reactionID);
		int count = 0;
		for (final IAtomContainer mol : reactantsorproducts.atomContainers()) {
