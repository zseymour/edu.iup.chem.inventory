...
public class SaveAsAction extends JCPAction {
	 ...
	 *            Description of the Parameter
	 */
	public SaveAsAction(final AbstractJChemPaintPanel jcpPanel,
			final boolean isPopupAction) {
	...
	public void actionPerformed(final ActionEvent event) {
		...
		final IChemModel jcpm = jcpPanel.getChemModel();
		if (jcpm == null) {
			final String error = GT._("Nothing to save.");
			JOptionPane.showMessageDialog(jcpPanel, error, error,
	...
	protected void saveAs(final ActionEvent event) {
		int ready = 1;
		while (ready == 1) {
			final IChemModel model = jcpPanel.getChemModel();
			...
			if (jcpPanel.getCurrentSaveFileFilter() != null) {
				for (int i = 0; i < chooser.getChoosableFileFilters().length; i++) {
					if (chooser.getChoosableFileFilters()[i].getDescription()
							.equals(jcpPanel.getCurrentSaveFileFilter()
									.getDescription())) {
			...
			final int returnVal = chooser.showSaveDialog(jcpPanel);

			final IChemObject object = getSource(event);
			final FileFilter currentFilter = chooser.getFileFilter();
			...
			if (returnVal == JFileChooser.APPROVE_OPTION) {
				if (!(currentFilter instanceof IJCPFileFilter)) {
					JOptionPane.showMessageDialog(jcpPanel,
							GT._("Please choose a file type!"),
	...
	protected File saveAsCDKSourceCode(final IChemModel model, File outFile)
			throws Exception {
		...
		while (containers.hasNext()) {
			final IAtomContainer ac = (IAtomContainer) containers.next();
			if (ac != null) {
				cow.write(ac);
			} else {
	...
	protected File saveAsCML2(final IChemObject object, File outFile)
			throws Exception {
		if (Float.parseFloat(System.getProperty("java.specification.version")) < 1.5) {
			JOptionPane.showMessageDialog(null,
					"For saving as CML you need Java 1.5 or higher!");
			return outFile;
		}
	...
	protected File saveAsMol(final IChemModel model, File outFile)
			throws Exception {
		logger.info("Saving the contents in a MDL molfile file...");

		if (model.getMoleculeSet() == null
				|| model.getMoleculeSet().getAtomContainerCount() == 0) {
			final String error = GT._("Problems handling data");
			final String message = GT
					._("MDL mol files can only save molecules. You have no molecules painted!");
			JOptionPane.showMessageDialog(jcpPanel, message, error,
					JOptionPane.WARNING_MESSAGE);
			return null;
		}
		if (model.getReactionSet() != null
			...
			final String error = GT._("Problems handling data");
			final String message = GT
					._("{0} files cannot contain reactions. Your have reaction(s) painted. The reactants/products of these will be included as separate molecules. Continue?",
							"MDL mol");
			final int answer = JOptionPane.showConfirmDialog(jcpPanel, message,
		...
		if (jcpPanel.get2DHub().getRGroupHandler() != null) {
			rGroupQuery = jcpPanel.get2DHub().getRGroupHandler()
					.getrGroupQuery();
		}

		if (rGroupQuery != null) {
			final String error = GT._("Please choose a file type!");
			final String message = GT
					._("Would you like to save the drawing as an R-group Query File? (RGFile = extended MOLfile)");
			final int answer = JOptionPane.showConfirmDialog(jcpPanel, message,
					error, JOptionPane.YES_NO_OPTION);
		...
		if (saveAsRgrpQuery) {
			...
			if (!rGroupQuery.areRootAtomsDefined()) {
				message = GT
						._("The R-group Query is not valid: there are substitutes that have no corresponding atom in the root structure.");
				problem = true;
			}
			if (!rGroupQuery.areSubstituentsDefined()) {
				message = GT
						._("The R-group Query is not valid: the root structure has R# definitions for which no substitutes are defined.");
				problem = true;
			}
			if (problem) {
				final String error = GT._("Could not save file");
				JOptionPane.showMessageDialog(jcpPanel, message, GT._(error),
						JOptionPane.INFORMATION_MESSAGE);
				return null;
			}
	...
	protected File saveAsRXN(final IChemModel model, File outFile)
			throws IOException, CDKException {
		if (model.getMoleculeSet() != null
				&& model.getMoleculeSet().getAtomContainerCount() > 0) {
			final String error = GT._("Problems handling data");
			final String message = GT
					._("{0} files cannot contain extra molecules. You painted molecules outside the reaction(s), which will not be in the file. Continue?",
							"RXN");
		...
		if (model.getReactionSet() == null
			...
			final String error = GT._("Problems handling data");
			final String message = GT
					._("RXN can only save reactions. You have no reactions painted!");
			JOptionPane.showMessageDialog(jcpPanel, message, error,
					JOptionPane.WARNING_MESSAGE);
			return null;
		}
	...
	protected File saveAsSMILES(final IChemModel model, File outFile)
			throws Exception {
		...
		while (containers.hasNext()) {
			...
			som.addAtomContainer(containers.next().clone());
		}
		cow.write(som);
		cow.close();
		if (jcpPanel instanceof JChemPaintPanel) {
