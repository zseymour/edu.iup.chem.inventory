...
/**
 * A panel containing a text field and button to directly insert SMILES or
 * InChI's
 * 
 */
public class InsertTextPanel extends JPanel implements ActionListener {
	...
	private final AbstractJChemPaintPanel	jChemPaintPanel;
	private final JComboBox					textCombo;
	private final JTextComponent			editor;
	private JFrame							closeafter			= null;
	private final JButton					button				= new JButton(
																		GT._("Insert"));

	public InsertTextPanel(final AbstractJChemPaintPanel jChemPaintPanel,
			final JFrame closeafter) {
		super();
		this.closeafter = closeafter;
		setLayout(new GridBagLayout());

		final List<String> oldText = new ArrayList<String>();
		oldText.add("");
	...
	private String getDataFromURL(final String url) throws IOException {
	...
	private IAtomContainer getMolecule() {
		...
		if (text.startsWith("InChI")) { // handle it as an InChI
			...
			//
			// } catch (Exception e2) {
			// JOptionPane.showMessageDialog(jChemPaintPanel,
			// GT._("Could not load InChI subsystem"));
			// return null;
			// }
			...
			try {
				...
				JOptionPane.showMessageDialog(jChemPaintPanel,
						GT._("Error in reading data from PubChem"));
				return null;
			}
			...
			} catch (final InvalidSmilesException e1) {
				JOptionPane.showMessageDialog(jChemPaintPanel,
						GT._("Invalid SMILES specified"));
				return null;
			}
			try {
				molecule = new FixBondOrdersTool()
						.kekuliseAromaticRings(molecule);
			} catch (final CDKException e) {
		...
		final String tmp = (String) textCombo.getItemAt(0);
		if (tmp.equals("")) {
			textCombo.removeItemAt(0);
		}
	...
	private IAtomContainer getMoleculeFromCAS(final String cas)
		...
		Pattern pattern = Pattern
				.compile("http://pubchem.ncbi.nlm.nih.gov/summary/summary.cgi\\?cid=(\\d*)");
		...
		final String secondURL = "http://eutils.ncbi.nlm.nih.gov/entrez/eutils/esummary.fcgi?tool=jcppubchem&db=pccompound&id="
				+ cid;
		data = getDataFromURL(secondURL);

		pattern = Pattern
				.compile("<Item Name=\"CanonicalSmile\" Type=\"String\">([^\\s]*?)</Item>");
		...
		try {
			...
			JOptionPane.showMessageDialog(jChemPaintPanel,
					"Couldn't process data from PubChem");
			return null;
		}
	}

	private boolean isCASNumber(final String text) {
